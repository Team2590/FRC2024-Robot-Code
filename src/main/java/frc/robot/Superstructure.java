  ...
  public void periodic() {
    switch (systemState) {
      ...
      case IDLE:
        ...
        climb.setStopped();
        arm.setHome();
        if (RobotContainer.poseEstimator.distanceToTarget() <= ShooterConstants.MAX_SHOOT_RANGE) {
          led.setRGB(13, 222, 48);
        } else if (conveyor.hasNote()) {
          led.setRGB(242, 129, 7);
        } else {
          led.setRGB(255, 0, 0);
        }
      ...
      case INTAKE:
        ...
        led.setFlashing(242, 129, 7, LEDConstants.FLASH_INTERVAL);
        if (arm.getState() == ArmStates.HOME) {
          intake.setIntake();
