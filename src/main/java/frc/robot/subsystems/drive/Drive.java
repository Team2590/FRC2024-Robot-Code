...
public class Drive extends SubsystemBase {
  ...
  public Drive(
    ...
    PhotonCamera camera = new PhotonCamera("SmallPhotonCamera");
    final double CAMERA_HEIGHT_METERS = 45.5 / 100;
    final double CAMERA_X_DISTANCE_FROM_CENTER_METERS = Units.inchesToMeters(12.5);
    final double CAMERA_Y_DISTANCE_FROM_CENTER_METERS = Units.inchesToMeters(.5);
    final double CAMERA_ROLL = 0;
    final double CAMERA_PITCH = Units.degreesToRadians(45);
    final double CAMERA_YAW = 0;
    final double APRILTAG_AMBIGUITY_THRESHOLD = 0.2;
    ...
    photonPoseEstimator =
        new PhotonPoseEstimator(
            layout, PoseStrategy.AVERAGE_BEST_TARGETS, cameraSim.getCamera(), RobotToCam.inverse());
  ...
  public void periodic() {
    ...
    PhotonPipelineResult latestResult = cameraSim.getCamera().getLatestResult();
    if (latestResult.hasTargets() && photonPoseEstimator.update(latestResult).isPresent()) {
      try {
        Logger.recordOutput(
            "Odometry/SimCameraPose", photonPoseEstimator.update(latestResult).get().estimatedPose);
      } catch (Exception e) {
